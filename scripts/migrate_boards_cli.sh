...
STAGE_HOST="${LOOKER_STAGE_BASE_URL:-}"
PROD_HOST="${LOOKER_PROD_BASE_URL:-}"
...
for BOARD_TITLE in $BOARDS; do
  ...
  STAGE_BOARD_JSON=$(looker-cli api board search_boards --token-file --host "$STAGE_HOST" 2>/dev/null || echo "[]")
  ...
  FULL_STAGE_BOARD=$(looker-cli api board board "$STAGE_BOARD_ID" --token-file --host "$STAGE_HOST")
  BOARD_DESC=$(echo "$FULL_STAGE_BOARD" | jq -r '.description // ""')

  # 2. Find or Create Board on Prod
  PROD_BOARD_JSON=$(looker-cli api board search_boards --token-file --host "$PROD_HOST" 2>/dev/null || echo "[]")
  ...
  if [ -z "$PROD_BOARD_ID" ] || [ "$PROD_BOARD_ID" = "null" ]; then
    ...
    NEW_PROD_BOARD=$(looker-cli api board create_board --token-file --host "$PROD_HOST" "{\"title\": \"$BOARD_TITLE\", \"description\": \"$BOARD_DESC\"}")
  ...
  for SEC in $SECTIONS; do
    ...
    PROD_SECS=$(looker-cli api board all_board_sections "$PROD_BOARD_ID" --token-file --host "$PROD_HOST" 2>/dev/null || echo "[]")
    ...
    if [ -z "$PROD_SEC_ID" ] || [ "$PROD_SEC_ID" = "null" ]; then
      ...
      NEW_SEC=$(looker-cli api board create_board_section --token-file --host "$PROD_HOST" "{\"board_id\": \"$PROD_BOARD_ID\", \"title\": \"$SEC_TITLE\", \"description\": \"$SEC_DESC\"}")
    ...
    for ITEM in $ITEMS; do
      ...
      if [ -n "$DASH_ID" ]; then
        DASH_TITLE=$(looker-cli api dashboard dashboard "$DASH_ID" --token-file --host "$STAGE_HOST" | jq -r '.title // empty')
        if [ -n "$DASH_TITLE" ]; then
          # Search for matching dashboard by title on Prod
          PROD_DASH_ID=$(looker-cli api dashboard search_dashboards --token-file --host "$PROD_HOST" | jq -r --arg t "$DASH_TITLE" '.[] | select(.title | ascii_downcase == ($t | ascii_downcase)) | .id' | head -n 1)
          if [ -n "$PROD_DASH_ID" ] && [ "$PROD_DASH_ID" != "null" ]; then
            echo "    Pinning Dashboard '$DASH_TITLE' (Prod ID: $PROD_DASH_ID) to section '$SEC_TITLE'..."
            looker-cli api board create_board_item --token-file --host "$PROD_HOST" "{\"board_section_id\": \"$PROD_SEC_ID\", \"dashboard_id\": \"$PROD_DASH_ID\"}" >/dev/null 2>&1 || true
          fi
        fi
      elif [ -n "$LOOK_ID" ]; then
        LOOK_TITLE=$(looker-cli api look look "$LOOK_ID" --token-file --host "$STAGE_HOST" | jq -r '.title // empty')
        if [ -n "$LOOK_TITLE" ]; then
          PROD_LOOK_ID=$(looker-cli api look search_looks --token-file --host "$PROD_HOST" | jq -r --arg t "$LOOK_TITLE" '.[] | select(.title | ascii_downcase == ($t | ascii_downcase)) | .id' | head -n 1)
          if [ -n "$PROD_LOOK_ID" ] && [ "$PROD_LOOK_ID" != "null" ]; then
            echo "    Pinning Look '$LOOK_TITLE' (Prod ID: $PROD_LOOK_ID) to section '$SEC_TITLE'..."
            looker-cli api board create_board_item --token-file --host "$PROD_HOST" "{\"board_section_id\": \"$PROD_SEC_ID\", \"look_id\": \"$PROD_LOOK_ID\"}" >/dev/null 2>&1 || true
          fi
        fi
      fi
    done
