...
AGENT_RESP=$(looker-cli api artifact artifact "$ARTIFACT_NAMESPACE" "$ARTIFACT_AGENT_KEY" --token-file --host "$TARGET_HOST" --port "$LOOKER_PORT" 2>/dev/null || echo "[]")
AGENT_MAP_VERSION=$(echo "$AGENT_RESP" | jq -r '.[0].version // empty' 2>/dev/null || true)
SOURCE_TO_TARGET_AGENT_MAP=$(echo "$AGENT_RESP" | jq -c '.[0].value // "{}" | if type == "string" then (fromjson? // {}) else . end' 2>/dev/null || echo "{}")
AGENT_MAP_MODIFIED=false

QUERY_RESP=$(looker-cli api artifact artifact "$ARTIFACT_NAMESPACE" "$ARTIFACT_QUERY_KEY" --token-file --host "$TARGET_HOST" --port "$LOOKER_PORT" 2>/dev/null || echo "[]")
QUERY_MAP_VERSION=$(echo "$QUERY_RESP" | jq -r '.[0].version // empty' 2>/dev/null || true)
SOURCE_TO_TARGET_QUERY_MAP=$(echo "$QUERY_RESP" | jq -c '.[0].value // "{}" | if type == "string" then (fromjson? // {}) else . end' 2>/dev/null || echo "{}")
...
while IFS= read -r AGENT_SUMMARY; do
  ...
  while IFS= read -r GQ; do
    ...
    if CREATE_GQ_RESP=$(echo "$GQ_PAYLOAD" | looker-cli api conversational_analytics create_golden_query - --token-file --host "$TARGET_HOST" --port "$LOOKER_PORT" 2>/dev/null); then
      ...
      echo "  Warning: Failed to create golden query on Target for Source golden query ID $GQ_ID." >&2
    fi
